 ...
 *
 * Updated: 10/3/2009
 * Validated KeyList sorting KMovA optimization where head of original list
	...
	for (KPtrA = RevList; KPtrA; KPtrA = KRefA->next)
	{
		...
		for (KPtrB = KPtrA->next; KPtrB; KPtrB = KRefB->next)
		{
			...
			if ((*KCompare)(KPtrB, KPtrA) <= 0)
			{
				...
				// Show that KPtrA has moved
				KMovA = KPtrB;
			}
			else
			{
				// The (!KMovA && KRefB) minimize the use of (*KCompare)
				if (!KMovA && KRefB && ((*KCompare)(KPtrB, KRefB) <= 0))
