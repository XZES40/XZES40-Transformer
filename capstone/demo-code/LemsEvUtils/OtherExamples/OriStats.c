...
//   OriDailyCount.csv
//
// Statistics Collection Lists
//
// KeyList (mkeCounts)
//   next -> (next month)
//   k_id = yrmon INTEGER
//   k_size = counts per month
//   k_next -> list of MKEs
//   \  next -> (next MKE)
//      k_name = the MKE
//      k_size = counts per MKE
//
// KeyList (countDay)
//   next -> (next month)
//   k_id = yrmon INTEGER
//   k_size = counts per month
//   k_next -> list of days
//   \  next -> (next day in month)
//      k_id = tm_mday  DAY_IN_MONTH
//      k_size = counts per Day
//
...
//					(char[4])		= theMessage->msgHdr->theIndex.MsgText;
						...
						(tmcomp(&logtime_tm, &endDateTime) <= 0))
					{
						if (theMessage->msgHdr->theIndex.Event_Id == INPUTMESSAGE &&
							theMessage->msgHdr->theIndex.ORI_Id[0] &&
							...
							(strcmp(theMessage->msgHdr->theIndex.InLine, "NCIC")) &&
